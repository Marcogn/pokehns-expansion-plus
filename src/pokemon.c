...
// modifyStats == FALSE asks "would this item do anything?" without committing
// the stat-changing part of the answer. It exists because the SwSh party menu
// probes an item first, then asks how many to use, then applies them scaled by
// itemCount; without it the probe applies the item for real and the mon gets
// the effect twice. Ported from Soulgold, which added both parameters.
// Only the writes Soulgold guards are deferred - EXP/level, EVs and friendship.
// HP, PP and status are applied by the probe and never re-applied, which is why
// the menu's follow-up call only exists for EV and candy items.
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, enum Item item, u8 partyIndex, u8 moveIndex, bool8 usedByAI, u8 modifyStats, u16 itemCount)
{
    ...
    enum Item heldItem;
    u8 effectFlags;
    s32 evChange; // was s8: it now has to hold the per-item delta times itemCount
    u16 evCount;
    u8 levelBefore;
        ...
        case 3:
             ...
             && GetMonData(mon, MON_DATA_LEVEL) != MAX_LEVEL)
            {
                ...
                if (param == 0) // Rare Candy
                {
                    ...
                    u32 targetLevel = min(level + itemCount, MAX_LEVEL);
