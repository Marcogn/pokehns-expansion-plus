...
// soulgold keeps this in item.c, where it also excludes its infinite-hold mega
// stones. Nothing here holds an item that cannot be tossed for that reason.
static bool32 CanItemBeTossed(enum Item itemId)
{
...
static bool8 ShouldLevelUpItemUseLevelCap(void)
{
    // soulgold reads the cap from a per-save option; here it is the
    // compile-time B_EXP_CAP_TYPE config.
    return B_RARE_CANDY_CAP && (B_EXP_CAP_TYPE == EXP_CAP_SOFT || B_EXP_CAP_TYPE == EXP_CAP_HARD);
}
