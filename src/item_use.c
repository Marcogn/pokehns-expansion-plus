...
// Ported from Soulgold unchanged apart from the item name in the message.
...
// The candies land in the Items pocket while the Key Items pocket is on screen,
// so that list has to be rebuilt before the bag is drawn again.
static void CloseCandyJarMessage(u8 taskId)
{
...
static u32 ConvertCandyJarExpToCandies(u8 *summaryDst)
{
    ...
    } sCandyInfo[] =
    {
        ...
        {ITEM_EXP_CANDY_M, EXP_CANDY_M_THRESHOLD},
...
void ItemUseOutOfBattle_PokeFlute(u8 taskId)
{
    ...
    {
        if (!ExecuteTableBasedItemEffect(&gPlayerParty[i], ITEM_AWAKENING, i, 0, TRUE, 1))
            wokeSomeoneUp = TRUE;
    }
